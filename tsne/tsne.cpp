...
MatrixXd x2p(const MatrixXd &X, const double tol, const double perplexity) {
  ...
  MatrixXd D = ((-2 * X * X.transpose()) + sum_X.replicate(1, X.rows())).transpose() + sum_X.replicate(1, X.rows());
...
MatrixXd tSNE(const MatrixXd &X, const int out_dims, const int init_dims,
  ...
  for (int iter = 0; iter < max_iter; iter++) {
    ...
    num = (1.0 / (((num + sum_Y.replicate(1, n)).transpose().eval() + sum_Y.replicate(1, n)).array() + 1.0)).matrix();
    ...
    gains = ((gains.array() + 0.2) * ((dY.array() > 0.0) != (iY.array() > 0.0)).cast<double>().array() +
            (gains.array() * 0.8) * ((dY.array() > 0.0) == (iY.array() > 0.0)).cast<double>().array()).matrix();
    ...
    if ((iter + 1) % 100 == 0) {
      ...
      std::cout << "# Iter: " << iter + 1 << ", err: " << c << std::endl;
    }
    if (iter == 100)
      P = P / 4;
  }
